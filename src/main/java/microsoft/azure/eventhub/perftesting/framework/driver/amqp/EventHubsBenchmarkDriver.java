 ...
 */
package microsoft.azure.eventhub.perftesting.framework.driver.amqp;
...
import microsoft.azure.eventhub.perftesting.framework.driver.base.BenchmarkConsumer;
import microsoft.azure.eventhub.perftesting.framework.driver.base.BenchmarkDriver;
import microsoft.azure.eventhub.perftesting.framework.driver.base.BenchmarkProducer;
import microsoft.azure.eventhub.perftesting.framework.driver.base.ConsumerCallback;
import microsoft.azure.eventhub.perftesting.framework.storageadapter.StorageAdapter;
...
public class EventHubsBenchmarkDriver implements BenchmarkDriver {
  ...
  public void initialize(DriverConfiguration driverConfiguration) throws IOException {
    ...
    blobContainerAsyncClient =
        StorageAdapter.GetAsyncStorageClient(
    ...
    eventHubAdministrator =
        EventHubAdministrator.getInstance(driverConfiguration.namespaceMetadata);

    if (Optional.ofNullable(driverConfiguration.reset).orElse(true)) {
    ...
    if (driverConfiguration.namespaceMetadata.sasKeyValue == null) {
      ...
      final EventHubNamespaceAuthorizationRule authorizationRule =
          eventHubAdministrator.getAuthorizationRule();
    ...
    credential =
        new EventHubSharedKeyCredential(
  ...
  public CompletableFuture<BenchmarkProducer> createProducer(String topic) {
    EventHubProducerAsyncClient ehProducerClient =
        new EventHubClientBuilder()
            .credential(
                namespace + configProvider.getConfigurationValue(ConfigurationKey.FQDNSuffix),
                topic,
                credential)
            .buildAsyncProducerClient();
    BenchmarkProducer benchmarkProducer =
        new EventHubsBenchmarkProducer(ehProducerClient, producerProperties);
    try {
  ...
  public CompletableFuture<BenchmarkConsumer> createConsumer(
    ...
    EventProcessorClient eventProcessorClient =
        new EventProcessorClientBuilder()
            ...
            .processEvent(
                eventContext ->
                    EventHubsBenchmarkConsumer.processEvent(eventContext, consumerCallback))
            .processError(
                errorContext ->
                    log.error(
                        "exception occur while consuming message "
